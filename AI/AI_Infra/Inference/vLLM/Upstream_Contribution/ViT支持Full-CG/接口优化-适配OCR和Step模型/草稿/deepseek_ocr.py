...
class DeepseekOCRForCausalLM(
    ...
    def _get_num_input_output_tokens(
        ...
        if is_tiled:
            num_input_patches = image_spatial_crop.prod(dim=-1)
            num_input_tokens += num_input_patches * (local_input_side ** 2)
        ...
        if is_tiled:
            num_output_tokens += (image_spatial_crop[0] * local_h) * (image_spatial_crop[1] * local_w)
    ...
    def get_encoder_cudagraph_item_specs(
        self,
        mm_kwargs: dict[str, Any],
    ) -> list[EncoderCudaGraphItemInfo]:
        item_specs = []
        for image_spatial_crop in mm_kwargs["images_spatial_crop"]:
            num_input_tokens, num_output_tokens = self._get_num_input_output_tokens(image_spatial_crop)
            item_specs.append(EncoderItemSpec(
    ...
    def select_encoder_cudagraph_items(
        ...
        selected_ic = torch.cat(
            [
                images_crop[cum_patches[i] : cum_patches[i + 1]]
                for i in indices
            ]
        )
    ...
    def prepare_encoder_cudagraph_capture_inputs(
        ...
        patch_token_budget = token_budget - max_batch_size * self.global_image_output_token
        max_num_patches = max(
            0,
    ...
    def postprocess_encoder_output(
        ...
        num_patches = [
            int(np)
            for np in torch.where(is_tiled, images_spatial_crop.prod(dim=-1), 0)
        ]
        ...
        local_part = (
            output[global_portion : global_portion + patch_portion]
            .reshape(total_patches, self.single_patch_output_token, n_embed)
            if patch_portion > 0
            else None
        )
        ...
        for i, idx in enumerate(indices):
            ...
            if num_patch > 0 and local_part is not None:
                ...
                patches = patches.reshape(
                    num_patch, patch_side, patch_side, n_embed
                )
