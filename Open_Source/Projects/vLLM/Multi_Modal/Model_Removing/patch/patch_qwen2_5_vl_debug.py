...
class AscendQwen2_5_VisionAttention(nn.Module):
    ...
    def forward(
        ...
        #     PRINT_ONCE = False
        #     print("-"*100)
        #     print(f"q.shape: {q.shape}")  # [1, 176, 8, 128]
        #     print(f"k.shape: {k.shape}")
        #     print(f"v.shape: {v.shape}")
        #     print(f"cos.shape: {cos.shape}")  # [1, 176, 1, 128]
        #     print(f"sin.shape: {sin.shape}")
        #     print(f"cu_seqlens before diff: {cu_seqlens}")  # vllm: [0, 64, 128, 176]
        #     cu_seqlens = torch.diff(cu_seqlens)
        #     print(f"cu_seqlens after diff: {cu_seqlens}")  # vllm-ascend: [64, 64, 48]
        #     print("-"*100)
        ...
        if PRINT_ONCE:
            PRINT_ONCE = False
            print("-"*100)
            # print(f"query.shape: {q.shape}")  # [176, 8, 128]
            # print(f"key.shape: {k.shape}")  # [176, 8, 128]
            # print(f"value.shape: {v.shape}")  # [176, 8, 128]
            print(f"query: {q}")
            print(f"key: {k}")
            print(f"value: {v}")
            print(f"seq_len: {cu_seqlens}")  # [64, 64, 48]
            # print(f"scale_value: {self.origin_hidden_size_per_attention_head**-0.5}")  # 0.11180339887498948
            # print(f"num_heads: {self.num_attention_heads_per_partition}")  # 8
            # print(f"num_kv_heads: {self.num_attention_heads_per_partition}")  # 8
            # print(f"out.shape: {context_layer.shape}")  # [176, 8, 128]
            print("-"*100)
